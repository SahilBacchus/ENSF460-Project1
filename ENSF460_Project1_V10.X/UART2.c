...
void Disp2Time(uint8_t minutes, uint8_t seconds, timer_state state)
{
    const char* prefix;

    // Handle ALARM state separately since it has different format
    ...
    // Set prefix for other states
    switch (state) {
        case STATE_SET_TIME: prefix = "SET "; break;
        case STATE_COUNTING: prefix = "CNT "; break;
        case STATE_PAUSED:   prefix = "PAU "; break;
        case STATE_CLEAR:    prefix = "CLR " ; break;
    ...
    XmitUART2('\r', 1);  // Carriage return to start of line
    
    Disp2String(prefix);

    
    // Display the prefix (SET, CNT, PAU, etc.)
    // Disp2String(prefix);
    
    // Display minutes (2 digits)
    while(ctr < 2) {
        quot = minutes / (pow(10, (1 - ctr)));
        rem = quot % 10;
        XmitUART2(rem + 0x30, 1);
        ctr = ctr + 1;
    }
    ...
    XmitUART2('s', 1);
    XmitUART2(' ', 1);  // Extra space for clearing previous text
    Disp2String("                       "); 
